...
# ============================================================
# تابع ساخت signature رسمی صرافی XT (مطابق مستندات doc.xt.com)
#
# فرمول:  original = X + Y
#   X = validate-algorithms=HmacSHA256&validate-appkey=KEY&validate-recvwindow=5000&validate-timestamp=TS
#   Y = #METHOD#/path           (اگر query و body خالی باشد)
#   Y = #METHOD#/path#query     (اگر فقط query داشته باشیم)
# ============================================================
def xt_sign(secret_key: str, access_key: str, method: str, path: str,
            timestamp: str, query: str = "", body: str = "") -> dict:
    recvwindow = "5000"
    X = (
        f"validate-algorithms=HmacSHA256"
        f"&validate-appkey={access_key}"
        f"&validate-recvwindow={recvwindow}"
    ...
    Y = "#".join(parts)
    original = X + Y
    sig = hmac.new(secret_key.encode(), original.encode(), hashlib.sha256).hexdigest()
    return {
...
def asset_select(key_suffix):
    s = st.selectbox("🪙 انتخاب ارز:", ["BTC", "ETH", "BNB", "SOL", "TON", "XRP", "ADA", "DOGE"],
                     key=f"sel_{key_suffix}")
    c = st.text_input("✍️ یا تایپ دستی نماد (اختیاری):", value="", key=f"cust_{key_suffix}").upper().strip()
...
# ============================================================
    ...
    if not st.session_state['xt_key'] or not st.session_state['xt_sec']:
        ...
        with st.spinner("🔄 در حال دریافت موجودی از صرافی XT..."):
            ...
            # ---- اسپات: GET https://sapi.xt.com/v4/balance?currency=usdt ----
            # طبق مستندات رسمی:
            #   query = "currency=usdt"
            #   Y = #GET#/v4/balance#currency=usdt
            try:
                ts = str(int(time.time() * 1000))
                path = "/v4/balance"
                query = "currency=usdt"
                headers = xt_sign(
                    st.session_state['xt_sec'],
                    st.session_state['xt_key'],
                    "GET", path, ts, query=query
                )
                res = requests.get(
                    f"https://sapi.xt.com{path}?{query}",
                    headers=headers, timeout=8
                )
                if res.status_code == 200:
                    d = res.json()
                    # ساختار پاسخ: {"rc":0,"result":{"currency":"usdt","available":"...","freeze":"..."}}
            ...
            # ---- فیوچرز: GET https://fapi.xt.com/future/v1/balance/list ----
            # فیوچرز از همان فرمت signature استفاده می‌کند
            try:
                ts = str(int(time.time() * 1000))
                path = "/future/v1/balance/list"
                headers = xt_sign(
                    st.session_state['xt_sec'],
                    st.session_state['xt_key'],
                    "GET", path, ts
                    ...
                    if isinstance(result, list):
                        for item in result:
                            if item.get("coin", "").upper() == "USDT":
                                usdt_futures = float(item.get("balance", 0))
                                break
                    elif not result:
                        pass  # حساب فیوچرز خالی است
                    else:
            ...
            <table class='custom-table'>
              <tr style='background:#1F2226;'>
                <th>بخش مالی صرافی XT</th>
                <th>موجودی واقعی (USDT)</th>
              </tr>
              <tr>
                <td style='color:#02C076;'>🟢 اسپات (Spot Wallet)</td>
                <td>{usdt_spot:,.2f} USDT</td>
              </tr>
              <tr>
                <td style='color:#F3BA2F;'>🔥 فیوچرز (Futures Account)</td>
                <td>{usdt_futures:,.2f} USDT</td>
              </tr>
              <tr style='background:#2B3139;'>
                <td style='color:#F3BA2F;font-size:18px;'>📊 جمع کل</td>
                <td style='color:#F3BA2F;font-size:18px;'>{total:,.2f} USDT</td>
              </tr>
            </table>
            """, unsafe_allow_html=True)
...
# ============================================================
    ...
    st.markdown("<div class='crypto-card-center'>", unsafe_allow_html=True)
    st.markdown("<h2 style='text-align:center;color:#F3BA2F;font-weight:900;'>💵 موجودی جزئی کیف پول‌ها</h2>", unsafe_allow_html=True)
            ...
            with st.spinner(f"در حال دریافت موجودی {chosen}..."):
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        if d.get("rc") == 0 and d.get("result"):
                            r = d["result"]
                            avail = float(r.get("available", 0)) if isinstance(r, dict) else 0
                            freeze = float(r.get("freeze", 0)) if isinstance(r, dict) else 0
                            st.markdown(f"""
                            <table class='custom-table'>
                              <tr style='background:#1F2226;'>
...
# ============================================================
    ...
    if st.button("📊 اسکن بازار"):
        with st.spinner("در حال دریافت قیمت از صرافی XT..."):
            try:
                res = requests.get(
