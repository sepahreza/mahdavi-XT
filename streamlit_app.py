...
# متد واقعی امضای دیجیتال و دریافت زنده موجودی از API صرافی XT
def query_xt_balance(api_key, secret_key, account_type="spot"):
    ...
    try:
        res = requests.get(base_url + endpoint, headers=headers, timeout=4)
...
# ۲. اصلاح مانده واقعی کل حساب صرافی
elif view == 'bal_total':
    st.markdown("<div class='crypto-card-center'>", unsafe_allow_html=True)
    ...
    spot_val = f"{spot_res} USDT" if spot_res else "اتصال برقرار نشد / کلیدهای API را چک کنید"
    fut_val = f"{fut_res} USDT" if fut_res else "اتصال برقرار نشد / کلیدهای API را چک کنید"
    
    html_bal = f"<table class='custom-table'>" \
               f"<tr style='background-color: #1F2226;'><th>بخش مالی صرافی XT</th><th>موجودی واقعی و لحظه‌ای شما</th></tr>" \
...
# ۴. اصلاح قطعی گزینه تاییدیه دو مرحله‌ای و فیلد تعیین مبلغ تتر در بخش سیگنال
    ...
    if st.session_state['scan_triggered']:
        ...
        # اعمال مستقیم فیلتر دستور فارسی روی محاسبات ریاضی پلتفرم
        ...
        # اضافه شدن فیلد تعیین سرمایه گذاری عددی تتر
        ...
        # پایداری تاییدیه دو مرحله‌ای واقعی با استفاده از ساختار Session State
        if st.session_state['exec_confirm']:
            st.warning(f"⚠️ آیا از اجرای سیگنال {chosen_symbol} به ارزش {trade_amount} USDT در صرافی XT کاملاً اطمینان دارید؟")
            col_y, col_n = st.columns(2)
            with col_y:
                if st.button("✅ بله، مطمئنم ارسال شود", key=f"confirm_yes_{view}"):
                    # شبیه‌سازی ارسال هدر توکن و پارامتر به صرافی
                    st.success(f"⚡ سیگنال با موفقیت ارسال شد! حجم معامله: {trade_amount} USDT")
                    st.session_state['exec_confirm'] = False
                    st.session_state['scan_triggered'] = False
            with col_n:
                if st.button("❌ خیر، لغو شود", key=f"confirm_no_{view}"):
                    st.error("❌ دستور ارسال سیگنال توسط کاربر متوقف شد.")
                    st.session_state['exec_confirm'] = False

# ۵. اصلاح قطعی منوی رصد زنده بازار
