...
# Y = #METHOD#path  یا  #METHOD#path#query  یا  #METHOD#path#body
# ============================================================
def spot_sign(secret: str, key: str, method: str, path: str,
    ...
    if body:
        parts.append(body)
    Y = "#".join(parts)
    sig = hmac.new(secret.encode('utf-8'), (X + Y).encode('utf-8'), hashlib.sha256).hexdigest()
    return {
...
# ============================================================
# FUTURES signature  ✅ اصلاح‌شده
# X = validate-appkey=KEY&validate-timestamp=TS
# Y = #METHOD#path?query   (query داخل path با ? نه جداگانه با #)
# original = X + Y
# ============================================================
def futures_sign(secret: str, key: str, method: str, path: str,
    ...
    # ✅ query string باید داخل path با ? باشد نه جداگانه
    full_path = f"{path}?{query}" if query else path
    Y = f"#{method}#{full_path}"

    # اگر body داشت (برای POST)
    if body:
        Y += f"#{body}"

    original = X + Y
    sig = hmac.new(secret.encode('utf-8'), original.encode('utf-8'), hashlib.sha256).hexdigest()
    return {
        "validate-appkey": key,
        "validate-timestamp": ts,
        "validate-signature": sig,
        "Content-Type": "application/json",  # ✅ فیوچرز JSON می‌خواهد
    }
