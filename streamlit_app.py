# مستندات رفع خطای API صرافی XT و نسخه نهایی اتاق فرمان پلتفرم

## ۱. علت ریشه‌ای خطای ۴۰۴ و پاسخ صرافی
در ساختار پلتفرم صرافی XT، آدرس‌های پایه‌ای (Base URLs) برای بخش‌های مختلف مجزا هستند. طبق مستندات، معاملات و اطلاعات حساب اسپات بر روی ساب‌دامنه `sapi.xt.com` قرار دارند. پیغام خطای زیر نشان‌دهنده عدم تطابق مسیر (`Path`) با آدرس سرور بود:
{"rc":1,"mc":"API not found, Please check the documentation","ma":[],"result":null}

راه‌حل فنی: تغییر مسیر درخواست مانده حساب اسپات در نسخه ۴ از `/v4/accounts` به آدرس دقیق و تست‌شده `/v4/balance` بر روی دامنه `sapi.xt.com` جهت دریافت مستقیم موجودی تتر (USDT).

---

## ۲. کد نهایی و اصلاح‌شده پلتفرم (streamlit_app.py)
...
if view == 'home':
    ...
    if not st.session_state['xt_key'] or not st.session_state['xt_sec']:
        ...
        with st.spinner("🔄 در حال استعلام نهایی موجودی با آدرس مپ شده..."):
            ...
            # --- اتصال به سرور رسمی اسپات با هندل کدهای نسخه ۴ ---
            try:
...
---
## ۳. دستور گیت برای اعمال تغییرات روی سرور
git add . && git commit -m "changed endpoint to v4 balance to prevent API not found error" && git push